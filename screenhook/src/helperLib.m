...
+ (void) toggleDock { //fn+a
    CGEventRef keyDownEvent = CGEventCreateKeyboardEvent(NULL, (CGKeyCode)0x00, true); // 0x00 is the key code for the "A" key
    CGEventSetFlags(keyDownEvent, kCGEventFlagMaskSecondaryFn);
    CGEventRef keyUpEvent = CGEventCreateKeyboardEvent(NULL, (CGKeyCode)0x00, false); // Release the "A" key
    CGEventSetFlags(keyUpEvent, kCGEventFlagMaskSecondaryFn);
...
+ (NSRunningApplication*) appWithBID: (NSString*) tarBID {
    ...
    if (apps.count > 1) NSLog(@"%@ has multiple NSRunningApplication's o_o", tarBID);
    if (apps.count) return apps[0]; else return nil;
}
...
+ (void) openAppExpose {CoreDockSendNotification(CFSTR("com.apple.expose.front.awake"), nil);}
+ (void) openDesktopExpose {CoreDockSendNotification(CFSTR("com.apple.showdesktop.awake"), nil);}
+ (void) openMissionControl { //and immediately show the icons at the top
    ...
    moveMouseTo(0, 0);
    CoreDockSendNotification(CFSTR("com.apple.expose.awake"), nil);
