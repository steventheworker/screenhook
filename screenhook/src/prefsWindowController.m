...
- (IBAction)modeChoice:(id)sender {
    int mode = [((NSButton*)sender).identifier isEqual: @"hoverModeBtn"] ? 1 : 2;
    [prefs setIntPref: @"previewMode" : mode];
//    [DockExpose setMode: mode];
    //click/ubuntu mode doesn't need mousemove (workaround for powerpoint notes bug where using ANY method to read the mouse coordinates causes the notes section to lose focus)
    [((AppDelegate*) NSApplication.sharedApplication.delegate)->app mousemoveLess: mode == 2 /* clickMode */ ? YES : NO];
}
...
//    [DockExpose setDelay: ((NSSlider*) sender).floatValue * 10 * 2 /* milliseconds */];
}
/* dock pref bindings */
